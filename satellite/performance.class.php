<?php
  ...
  class performance {
    ...
    private $_api_url = 'https://www.webhosting-performance.com/api/';
    ...
    // Bandwidth
      $this->perform_upstream_test(256000);
      $this->perform_downstream_test(256000);
    ...
    // Attempt #1 - Official
      $response = $this->http_request('https://www.webhosting-performance.com/api/get_country');
      if (preg_match('#^[A-Z]{2}$#', $response)) {
        file_put_contents('country.dat', $response);
        return $response;
      }

    // Attempt #2 - Freegeoip.net
    ...
    */
    ...
    // --------------------------------------------------------------
      ...
      for ($i=0; $i < $cycles; $i++) {
        $database->query(
          "INSERT INTO `". $database->table_prefix ."test` (`column`)
          VALUES ('". str_pad($i, strlen($cycles), '0', STR_PAD_LEFT) ."');"
        );
      }
    ...
    // --------------------------------------------------------------
      ...
      $measure_amount = $filesize / $time_elapsed / 1000000;
    ...
    public function perform_upstream_test($size=256000, $country_code=null) {
    ...
    public function perform_downstream_test($size=256000, $country_code=null) {

      if (empty($country_code)) $country_code = $this->_country_code;

      switch($country_code) {
        default:
          $url = 'https://www.webhosting-performance.com/images/sample.jpg'; // CDN
          break;
      }
    ...
    public function update($silent=false) {
      ...
      if (($response = json_decode($response, true)) === false ) {
        trigger_error("Unexpected response data from $url", E_USER_ERROR);
      }

      if (strtolower($response['status']) != 'ok') {
        trigger_error("Invalid response ({$response['status']})", E_USER_ERROR);
      }
